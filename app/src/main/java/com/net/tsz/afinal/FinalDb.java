...
import com.net.tsz.afinal.db.sqlite.CursorUtils;
import com.net.tsz.afinal.db.sqlite.DbModel;
import com.net.tsz.afinal.db.sqlite.OneToManyLazyLoader;
import com.net.tsz.afinal.db.sqlite.SqlBuilder;
import com.net.tsz.afinal.db.sqlite.SqlInfo;
import com.net.tsz.afinal.db.table.KeyValue;
import com.net.tsz.afinal.db.table.ManyToOne;
import com.net.tsz.afinal.db.table.OneToMany;
import com.net.tsz.afinal.db.table.TableInfo;
import com.net.tsz.afinal.exception.DbException;
import com.net.tsz.afinal.utils.ClassUtils;
...
public class FinalDb {
	...
	public FinalDb(SQLiteDatabase db) {
		this.db = db;
	}
	...
	public void deleteTable(Class<?> clazz) {
		debugSql("DROP TABLE IF EXISTS " + ClassUtils.getTableName(clazz));
		db.execSQL("DROP TABLE IF EXISTS " + ClassUtils.getTableName(clazz));
		TableInfo.remove(User.class);
	}
	...
	public void checkTableExist(Class<?> clazz) {
		if (!tableIsExist(TableInfo.get(clazz))) {
			...
			debugSql(sql);
			db.execSQL(sql);
			String indexSql=SqlBuilder.getIndexSQL(clazz);
	...
	public static class DaoConfig {
		...
		private String mDbName = StaticFactory.DBName; // 数据库名字
		private int dbVersion = StaticFactory.DBVersion; // 数据库版本
