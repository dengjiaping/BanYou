 ...
 */
package com.net.tsz.afinal.db.table;
...
import com.net.tsz.afinal.utils.FieldUtils;
...
public class TableInfo {
	...
	public final HashMap<String, Index> IndexMap = new HashMap<String, Index>();
	...
	public static boolean remove(Class<?> clazz){
		if(clazz==null)
		...
		if(tableInfo!=null){
			...
			for (String key : tableInfoMap.keySet()) {
				TableInfo ti=tableInfoMap.get(key);
				if(ti.getTableName().equals(ClassUtils.getTableName(clazz))){
					temp=key;
					break;
				}
			}
			if(temp!=null){
				tableInfoMap.remove(temp);
			}
		}
		return true;
	}
	...
	public static  TableInfo get(Class<?> clazz){
		...
		if( tableInfo == null ){
			...
			List<Index> iList=ClassUtils.getIndexList(clazz);
			if(iList!=null){
				for (Index index : iList) {
					if(index!=null)
						tableInfo.IndexMap.put(index.getColumn(), index);
				}
			}
