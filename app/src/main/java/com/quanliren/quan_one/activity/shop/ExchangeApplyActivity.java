...
public class ExchangeApplyActivity extends BaseActivity{
	...
	@ViewInject(id=R.id.card) EditText card;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.exchange_apply);
		setTitleTxt("换现金");
		setTitleRightTxt("完成");
	}
	...
	public void rightClick(View v) {
		...
		final String str_card=card.getText().toString().trim();
		...
		if(!Util.isStrNotNull(str_name)){
			...
			return;
		}else if(!Util.isStrNotNull(str_card)){
			showCustomToast("请输入支付宝账号");
			return;
		}else if(Util.isStrNotNull(str_email)&&!Util.isEmail(str_email)){
